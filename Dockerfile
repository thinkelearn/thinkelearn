...
#   2. Set up initial pages and showcase content.
...
CMD ["bash", "-c", "set -xe; python manage.py migrate --noinput; python manage.py setup_pages; python manage.py setup_showcase; python manage.py setup_portfolio; gunicorn thinkelearn.wsgi:application"]
