...
(function () {
    ...
    const initUploadForm = () => {
        ...
        submitBtn.addEventListener("click", async () => {
            ...
            const file = fileInput.files[0];
            if (!file) {
                showError("Please select a H5P package (.h5p) file.");
                return;
            }
            ...
            try {
                ...
                // (Mirrors the pattern used by Wagtail's own forceReload() helper.)
                ...
                showError(
                    "Network error during finalization. The file was uploaded but processing failed."
                );
                setSubmitEnabled(true);
            }
        });
    ...
    // Wagtail renders extra_js before form HTML, so wait for the DOM before
    // querying upload fields.
    if (document.readyState === "loading") {
        document.addEventListener("DOMContentLoaded", initUploadForm, { once: true });
    } else {
        initUploadForm();
    }
})();
